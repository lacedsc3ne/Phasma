...
namespace PhasmaStrap.UI.Elements.Settings.Pages
{
    public sealed class HotkeyCaptureSession
    {
        ...
        private readonly IReadOnlyList<HotkeyRow> _rows;
        ...
        public HotkeyCaptureSession(FrameworkElement owner, IReadOnlyList<HotkeyRow> rows)
        {
            _owner = owner;
            _rows = rows;
            ...
            _listenTimeout.Tick += (_, _) => Stop("Cancelled - no key was pressed.");
        ...
        public void Toggle(HotkeyRow row)
        {
        ...
        public void Clear(HotkeyRow row)
        {
            Stop(null);
        ...
        private void Window_Deactivated(object? sender, EventArgs e) => Stop(null);
        ...
        private void Finish(HotkeyRow row, string? message)
        {
            _listenTimeout.Stop();
            _modifiersDown.Clear();
        ...
        private bool OnKey(int vk, bool isDown)
        {
            ...
            if (_listening is null)
            {
                ...
                if (!isDown && vk == _drainKey)
                    _owner.Dispatcher.BeginInvoke(new Action(EndDrain));
        ...
        private void Commit(HotkeyRow row, ModifierKeys modifiers, int vk)
        {
            ...
            foreach (HotkeyRow other in _rows)
            {
            ...
            Finish(row, movedFrom is null ? null : $"Moved here from \"{movedFrom}\".");
        }

        private static void Describe(HotkeyRow row, string? prefix)
        {
